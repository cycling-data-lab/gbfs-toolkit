...
def main() -> None:
    ...
    panel = pd.DataFrame(
        {"system_id": "s", "station_id": "a", "fetched_at": times,
    ...
    weather = pd.DataFrame(
        {"time": pd.date_range("2026-06-01T06:00Z", periods=12, freq="1h"),
         "temp_c": np.linspace(14, 26, 12), "rain_mm": [0, 0, 0, 1, 2, 0, 0, 0, 0, 0, 3, 1]}
    )
    ...
    print(enriched[["fetched_at", "num_bikes_available", "temp_c", "rain_mm"]].head().to_string(index=False), "\n")

    # 3. The autocorrelation a model must respect (here at 1 h and 2 h lags).
    print("Temporal autocorrelation of availability:")
    print(gb.temporal_autocorrelation(panel, lags=(1, 2), freq="1h").round(3).to_string(index=False))
