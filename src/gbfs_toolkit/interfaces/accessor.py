...
from gbfs_toolkit.analytics import distribution, dynamics, fleet, frames, service, temporal
from gbfs_toolkit.core import models
from gbfs_toolkit.io import timeseries
from gbfs_toolkit.spatial import analytics as spatial_analytics
...
class GBFSAccessor:
    ...
    def occupancy(self) -> pd.Series:
        return frames.occupancy(self._df)

    def station_state(self) -> pd.Series:
        return frames.station_state(self._df)
    ...
    def system_profile(self) -> pd.Series:
        return distribution.system_profile(self._df)

    def concentration_metrics(self, **kw) -> pd.Series:
        return distribution.concentration_metrics(self._df, **kw)

    def coverage_stats(self, **kw) -> pd.Series:
        return spatial_analytics.coverage_stats(self._df, **kw)

    def availability_stats(self, **kw) -> pd.DataFrame:
        return temporal.availability_stats(self._df, **kw)

    def morans_i(self, value_col: str, **kw) -> pd.Series:
        return spatial_analytics.morans_i(self._df, value_col, **kw)
    ...
    def join_status(self, status: pd.DataFrame) -> pd.DataFrame:
        ...
        return frames.join_availability(self._df, status)
    ...
    def capacity_utilization(self, info: pd.DataFrame) -> pd.DataFrame:
        return service.capacity_utilization(self._df, info)

    def two_step_fca(self, demand: pd.DataFrame, **kw) -> pd.Series:
        return spatial_analytics.two_step_fca(self._df, demand, **kw)

    def join_exogenous(self, exogenous: pd.DataFrame, **kw) -> pd.DataFrame:
        return temporal.join_exogenous_timeseries(self._df, exogenous, **kw)

    # -- advanced analytics (1.4.0) -----------------------------------------
    def local_morans_i(self, value_col: str, **kw) -> pd.DataFrame:
        return spatial_analytics.local_morans_i(self._df, value_col, **kw)

    def diurnal_bimodality(self, **kw) -> pd.DataFrame:
        return temporal.diurnal_bimodality(self._df, **kw)

    def availability_synchrony(self, **kw) -> pd.DataFrame:
        return temporal.availability_synchrony(self._df, **kw)

    def outage_survival(self, **kw) -> pd.DataFrame:
        return service.outage_survival(self._df, **kw)

    def temporal_concentration(self, **kw) -> pd.DataFrame:
        return temporal.temporal_concentration(self._df, **kw)
